...
def main(cfg):
    ...
    for data_ind in tqdm(range(len(dataset))):
        ...
        #=======================================================================
        # find_error=['\n He in rich, Pfeiffer', 'Jorge Delgado', 'Ted Brannon', 'Ann Drummond', 'Wilhelm Falk', 'Amy Blackwood', 'Benedetto Luciano', 'Najaf Mansoor', 'Heinz Albrecht Vogler', 'Elsa Nordström', 'Philip Gresham', 'Theopompus Philotheou', 'Giovanni Carbone', 'Rudolf Engel', 'Hartmann Liebig', 'Adrienne Chastain', 'Siegfried Drescher', 'Moshe Itzik', 'Paul Marston', 'Alfred Barrow']
        # name=[]
        # for person in find_error:
        #     words = person.split()
        #     print(f"words : {words}")
        #     person_first_token_ids = [tokenizer(tok, add_special_tokens=False)['input_ids'] for tok in words]
        #     if len(person_first_token_ids[0]) < 2 or len(person_first_token_ids[1]) < 2 :
        #         if len(person_first_token_ids[0]) < 2:
        #             name.append(person+" name")
        #         if len(person_first_token_ids[1]) < 2 :
        #             name.append(person + " title")
        #     print(f"person_first_token_ids : {person_first_token_ids}")
        #     words2 = [f"\n{tok}" for tok in words]
        #     person_first_token_ids_2 = [tokenizer(tok, add_special_tokens=False)['input_ids'] for tok in words2]
        #     print(f"person_first_token_ids_2 : {person_first_token_ids_2}")
        # print(f"Person name with 1 tokens : {name}")
        #=======================================================================
        
        # prepare for name change
        # to remove index error for single tokenized name 
        alter_name= ['Ted Brannon', 'Ann Drummond', 'Amy Blackwood', 'Elsa Nordström', 'Paul Marston'] # collect by running above script
        # new_name = [name for name in replace_persons if name not in remove_name]
        new_name=[]
        for name in replace_persons:
            if name in alter_name:
                new_name.append(name.split()[0]+"eva"+" "+name.split()[1])
            else:
                new_name.append(name)
        replace_persons = new_name
        print(f"{replace_persons}")
        replace_person_first_token_ids = []
        i=0
        for person in replace_persons:
            print(f"Processing : {person}")
            words = person.split()
            # print(f"{tokenizer(words[0], add_special_tokens=False)['input_ids']}")
            # print(f"{tokenizer(words[1], add_special_tokens=False)['input_ids']}")
            person_first_token_ids = {tokenizer(tok, add_special_tokens=False)['input_ids'][0] for tok in words}
            words2 = [f"\n{tok}" for tok in words]
            # print(f"wordss2-0{tokenizer(words2[0], add_special_tokens=False)['input_ids']}")
            # print(f"wordss2-1{tokenizer(words2[1], add_special_tokens=False)['input_ids']}")
            person_first_token_ids_2 = {tokenizer(tok, add_special_tokens=False)['input_ids'][2] for tok in words2}
            # print(f"replace_persons : {replace_persons}")
            # print(f"words : {words}")
            # print(f"person_first_token_ids : {person_first_token_ids}")
            # print(f"words2 : {words2}")
            # print(f"tokenizer word{tokenizer(words2[0], add_special_tokens=False)['input_ids']}")
            # print(f"person_first_token_ids_2 : {person_first_token_ids_2}")
            # raise RuntimeError("error")
            # ===================================================================================
            # it throws error because of faulty comparison , it comapares if name a sub-word starts with a word ex. sh starts with rishav ?
            ...
            #     print(f"{tokenizer.decode(tok_id, skip_special_tokens=True)}")
            #     print(f" Assret {any([w.startswith(tokenizer.decode(tok_id, skip_special_tokens=True)) for w in words])}")
            ...
            print(f"Processed : {replace_person_first_token_ids[i]}")
            i=i+1
        ...
        for content_ind, content in enumerate(contents):
            ...
            if not teacher_cfg.whp_baseline and teacher_cfg.change_name_back:
                ...
                print(f"{tokenizer.decode(original_w_prefix, skip_special_tokens=True)}")
                print(f"{tokenizer.decode(original_wo_prefix, skip_special_tokens=True)}")
